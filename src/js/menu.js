...
function updateScoreboard() {
    // Simulação de dados - em produção, buscar do backend/localStorage
    const totalScore = Math.floor(Math.random() * 500); // Substituir por dados reais
    const completedChallenges = Math.floor(Math.random() * 7);
    ...
    if (totalProgressElement) {
        ...
        if (progressText) {
            progressText.textContent = `${totalScore} / 1000 pontos`;
        }
    }
    ...
    const startDate = new Date('2025-12-08T09:00:00');
    const now = new Date();
